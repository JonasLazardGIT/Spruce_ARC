...
# Flags
write_file = True
# Toggle the expensive MDS trail checks. Note: for t=98 this can take a very long time.
enable_checks = True
...
USE_FIXED_ROUNDS = False

# Optional args: [lenkey lennonce lentag] [fixed] [nochecks]
extra_args = sys.argv[8:]
numeric_args = []
flag_args = []
for a in extra_args:
    if a in ["fixed", "nochecks"]:
        flag_args.append(a)
    else:
        numeric_args.append(a)

if len(numeric_args) not in [0, 3]:
    print("Usage: <script> <field> <s_box> <field_size> <num_cells> <alpha> <security_level> <modulus_hex> [lenkey lennonce lentag] [fixed] [nochecks]")
    exit(1)

if len(numeric_args) == 3:
    LENKEY = int(numeric_args[0])
    LENNONCE = int(numeric_args[1])
    LENTAG = int(numeric_args[2])
    if LENKEY + LENNONCE != NUM_CELLS:
        print("ERROR: lenkey+lennonce (%d) does not match num_cells (%d)" % (LENKEY + LENNONCE, NUM_CELLS))
        exit(1)
else:
    LENNONCE = DEFAULT_LENNONCE
    LENTAG = DEFAULT_LENTAG
    LENKEY = NUM_CELLS - LENNONCE
    if LENKEY <= 0:
        print("ERROR: num_cells too small for default lennonce (%d)" % (LENNONCE))
        exit(1)

if "fixed" in flag_args:
    USE_FIXED_ROUNDS = True
if "nochecks" in flag_args:
    enable_checks = False

if USE_FIXED_ROUNDS:
    # Paper/example values to avoid excessive rounds.
    R_F_FIXED = 8
    R_P_FIXED = 10
else:
...
trunc_bound = float(SECURITY_LEVEL) / log(PRIME_NUMBER, 2)
perm_bits = NUM_CELLS * log(PRIME_NUMBER, 2) / 3.0
if NUM_CELLS - LENTAG <= trunc_bound:
    print("ERROR: truncation bound violated: t - lentag <= λ/log2(q)")
    exit(1)
if perm_bits < SECURITY_LEVEL:
    print("WARNING: permutation heuristic below target: est_bits=%.2f < %d" % (perm_bits, SECURITY_LEVEL))
...
print("PRF lengths: lenkey=%d lennonce=%d lentag=%d (t=%d)" % (LENKEY, LENNONCE, LENTAG, NUM_CELLS))
print("Truncation bound λ/log2(q)=%.2f, t-lentag=%d" % (trunc_bound, NUM_CELLS - LENTAG))
print("Permutation heuristic bits≈%.2f" % (perm_bits))
...
def print_round_constants(round_constants, n, field):
    # Keep the summary small to avoid huge stdout/IO.
...
def generate_matrix(FIELD, FIELD_SIZE, NUM_CELLS):
    """
    If enable_checks is True, run the original Algorithm 1–3 trail checks in a
    rejection loop (can be extremely slow for large t). Otherwise, return a
    single Cauchy MDS matrix directly.
    """
    if enable_checks == False:
        if FIELD == 0:
            return create_mds_gf2n(FIELD_SIZE, NUM_CELLS)
        else:
            return create_mds_p(FIELD_SIZE, NUM_CELLS)

    # Slow, check-enabled path
    if FIELD == 0:
...
def print_linear_layer(M, n, t):
    ...
    print("t:", t)
    print("N:", (n * t))
    # Skip the very expensive Algorithm 1–3 checks for large t; rely on Cauchy construction.
    if write_file == True:
        FILE.write("n: " + str(n) + "\n")
        FILE.write("t: " + str(t) + "\n")
...
# Export JSON parameters for Go
if True:
    t = NUM_CELLS
    cext = [round_constants[i*t:(i+1)*t] for i in range(R_F_FIXED)]
    cint = round_constants[R_F_FIXED*t:R_F_FIXED*t+R_P_FIXED]
    if LENKEY + LENNONCE != t:
        print("Warning: LENKEY+LENNONCE != t; adjusting LENKEY to t-lennonce")
        LENKEY = t - LENNONCE
    json_out = {
        "q": int(PRIME_NUMBER),
        "d": int(ALPHA),
        "lenkey": int(LENKEY),
        "lennonce": int(LENNONCE),
        "lentag": int(LENTAG),
        "t": int(t),
        "RF": int(R_F_FIXED),
        "RP": int(R_P_FIXED),
        "sec_trunc_bound": float(trunc_bound),
        "sec_perm_bits": float(perm_bits),
        # use same matrix for ME and MI as a default; can regenerate MI separately if desired
        "ME": [[int(e) for e in row] for row in linear_layer.rows()],
        "MI": [[int(e) for e in row] for row in linear_layer.rows()],
