...
\end{itemize}
The pre-sign proof is dominated by carrier membership, linear replay on decoded
aliases, and the transform-domain public-target hash using the public
theta-lift of the blind-signature target together with the committed hats
\(\widehat M,\widehat K,\widehat R_0,\widehat R_1\).
The post-sign proof introduces higher effective degree through the signature
shortness gadget, the direct limb-to-\(\widehat T_0\) bridge, and the PRF's
S-box constraints; it no longer commits a semantic witness row for the hash
target.
Appendix~\ref{app:smallwood} and Appendix~\ref{app:ext-params} detail how
