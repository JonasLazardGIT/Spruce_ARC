...
This appendix records the SmallWood proof stack underlying our ARC
instantiation. Section~\ref{sec:smallwood-model} fixes the statement layer:
which witness rows are committed, which rows remain virtual, and which
constraint families are replayed. The present appendix records the proof-system
mechanics that make those statements meaningful: degree enforcement, linear
openings, batched replay, and Fiat--Shamir compilation.
...
\begin{itemize}[leftmargin=1.25em]
  ...
  signature shortness, the direct limb-to-\(\widehat T_0\) bridge, the
  non-sign bridges to \(\widehat m,\widehat R_0,\widehat R_1\), the replay
  residual, and the PRF companion constraints.
...
The pre-sign witness consists of
\[
...
element \(T\), and the verifier derives its public theta-lift \(T^\Theta\). The
verifier requests openings sufficient to replay:
\begin{enumerate}[leftmargin=1.25em]
  ...
  decoded rows,
  \item commitment, centering, and packing on the decoded rows,
...
The showing witness consists of:
\begin{enumerate}[leftmargin=1.25em]
  \item signature limb rows \(L_{c,b,\ell}\),
...
the committed carriers, and the packed signature source remains a virtual linear
function of the limb rows. The direct bridge to \(\widehat T_0\) is what allows
the verifier to replay the showing equation from one committed signature-side
row.

\paragraph{The \(\Omega\)-sum check.}
The \(\Omega\)-sum check is active in both proofs. In issuance it enforces the
four non-sign transform bridges. In showing it enforces the direct
limb-to-\(\widehat T_0\) bridge together with the three non-sign transform
bridges. Thus the \(\Sigma_\Omega\) check is part of the statement layer in
both issuance and showing.
