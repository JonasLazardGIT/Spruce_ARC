...
| theorem-clean replay geometry | Paper fact: the strongest theorem-facing statement uses the full replay image. | Code fact: `go run ./cmd/showing -showing-preset compact_l1_research -full` is maintained on the checked-in canonical vector-`x0` artifacts and keeps the direct `bb_tran` replay image. | aligned | The default command remains the smaller reduced engineering path. |
