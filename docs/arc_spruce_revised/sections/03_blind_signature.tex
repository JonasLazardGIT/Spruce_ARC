...
\end{equation}
Whenever $B_3-r_1\in\Rq^{\times n}$, the holder computes the inverse witness
\[
  Z=(B_3-r_1)^{-1}
\]
...
\begin{align}
  ...
  (B_3-r_1)\Had Z &= 1, \label{eq:issuebs-rel-inverse}\\
...
We now record the shared-randomness blind-issuance protocol together with a raw post-issuance verification algorithm for the completed witness relation.
...
\begin{enumerate}[leftmargin=1.25em]
  ...
  \eqref{eq:bs-centered-randomness}.  If $B_3-r_1\notin\Rq^{\times n}$, the holder aborts the current issuance and restarts from the issuer-randomness round with fresh issuer randomness.  Otherwise it sets
  \[
    Z=(B_3-r_1)^{-1},
    \qquad
    T=B_0+B_1\vecmu+B_2r_0+Z.
...
\begin{proposition}[Issuer-view hiding of the issuance surface]\label{prop:bs-issuer-view-hiding}
Assume:
\begin{enumerate}[leftmargin=1.25em]
...
We show that the issuer's public view in an honest issuance to $\vecmu_b$ is computationally independent of
$b$.

First replace the real pre-sign proof $\nizkIssue$ by a simulated proof for the same public statement.  By zero
knowledge, this changes the issuer's distinguishing advantage by at most a negligible term.  After this step,
the proof transcript is no longer a possible leakage channel, so the relevant public view is reduced to
\[
  (\vecc,r_0^I,r_1^I,T).
\]
...
\[
  ...
  r_1=\centerfunc(r_1^H+r_1^I)
\]
are distributed exactly as fresh uniform bounded values over the same domain.  Hence, conditioned on any
fixed hidden holder-side randomness, the final pair $(r_0,r_1)$ has the protocol distribution required by the
LHL-based target-hiding theorem.  More precisely, Corollary~\ref{cor:htran-protocol-r0} shows that the
protocol distribution of $r_0$ is exactly the coefficient-wise uniform bounded distribution required by
Theorem~\ref{thm:htran-target-hiding}.

For honestly generated public parameters, and in particular for uniformly sampled $B_2$, Theorem~\ref{thm:htran-target-hiding} implies that for every fixed admissible value of $r_1$ the public target
\[
  T=B_0+B_1\vecmu_b+B_2r_0+(B_3-r_1)^{-1}
\]
statistically hides $\vecmu_b$.  Averaging over the independent centered distribution of $r_1$ preserves the
same hiding bound.  Since $r_0^I$ and $r_1^I$ are already independent of $b$, combining commitment hiding,
independent issuer randomness, and the LHL-based target-hiding theorem proves that the whole public issuance
surface is hiding.
\end{proof}

\begin{remark}[Security status]\label{rem:bs-security-status}
The non-blind signature layer inherits the DKLW sEUF-SMA and sEUF-CMA guarantees of
Section~\ref{subsec:prelim-signatures}.  The blind issuance protocol established in this section provides
correctness and issuer-view hiding.  A separate reduction is still needed for one-more unforgeability of the
