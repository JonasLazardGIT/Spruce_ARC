...
coefficient bound for messages and protocol randomness is denoted $\BoundMsg$, and the short-preimage bound
for signatures is denoted $\BoundSig$.
...
Let the commitment-only randomness $\bar r$ be sampled coefficient-wise from $[-B_r,B_r]$.  If
\[
  N\ell_{\bar r}\log_2(2B_r+1)
  \ge
  N\ell_{\ComScheme}\log_2 q + 2\lambda,
\]
then the commitment distribution is statistically $2^{-\lambda}$-close to a distribution independent of the
committed vector $(\vecmu,r_0^H,r_1^H)$.  In the bounded instantiation used here, one may take $B_r=\BoundMsg$.
\end{lemma}

\begin{proof}
For fixed $(\vecmu,r_0^H,r_1^H)$, the commitment is a public offset plus the linear image
$\mA_{\bar r}\bar r$.  Since $\mA_{\bar r}$ is uniform and the randomness block has the stated min-entropy,
the leftover hash lemma implies statistical closeness to uniform over the output space up to error $2^{-\lambda}$.
\end{proof}
...
\begin{itemize}[leftmargin=1.25em]
  \item $\PublicParamsTrapdoor\gets\Setup(\SecParam,\PublicParamsGen)$ outputs admissible public parameters,
  including dimensions $(n,m)$, modulus $q$, and the accepted shortness bound $\BoundSig$.
  \item $(\mA,\trapdoor)\gets\TrapGen(\PublicParamsTrapdoor)$ outputs a public matrix
  $\mA\in\Rq^{n\times m}$ together with a trapdoor $\trapdoor$, and the distribution of $\mA$ is statistically
  close to uniform over $\Rq^{n\times m}$.
  \item On input $(\PublicParamsTrapdoor,\trapdoor,T)$ with $T\in\Rq^n$, the sampler
  $\SampPre(\trapdoor,T)$ outputs $\vecu\in\R^m$ such that $\mA\vecu=T$ in $\Rq^n$, except with negligible
  failure probability, and $\|\vecu\|_\infty\le \BoundSig$ except with negligible probability.
\end{itemize}
\end{definition}
...
\]
In ARC-SPRUCE notation, the public description is
\[
  B=(B_0,B_1,B_2,B_3),
\]
where $B_0,B_3\in\Rq^n$ and $B_1,B_2$ are public linear maps into $\Rq^n$.  For message $\vecmu$ and
randomness $(r_0,r_1)$, define the linear side
\begin{equation}
  L_B(\vecmu,r_0):=B_0+B_1\vecmu+B_2r_0.
  \label{eq:htran-linear-side}
\end{equation}
The input $(B,\vecmu,r_0,r_1)$ is admissible if $B_3-r_1\in\Rq^{\times n}$, equivalently if every CRT slot of every
coordinate of $B_3-r_1$ is non-zero.  When admissible, let
\[
  Z=(B_3-r_1)^{-1}
\]
...
\end{equation}
The witness form is preferred in the protocol statements because it isolates the inverse check from the linear
identity for the public target.

\begin{lemma}[Eliminated form under admissibility]\label{lem:hash-cleared-admissible}
Let $\Rq\cong\prod_i F_i$ be the CRT decomposition and write $B_3-r_1=(d_i)_i$ and
...
Definition~\ref{def:htran-long-r0}.  If
\[
  N\ell_{r_0}\log_2(2\BoundMsg+1)
  \ge
  Nn\log_2 q + 2\lambda,
\]
then the joint distribution $(B_2,B_2r_0)$ is statistically $2^{-\lambda}$-close to
\[
  (B_2,U),\qquad U\getsunif \Rq^n.
\]
\end{lemma}

\begin{proof}
The map $r_0\mapsto B_2r_0$ is a linear hash family over the cyclotomic ring.  The source has min-entropy
$N\ell_{r_0}\log_2(2\BoundMsg+1)$, while the output space $\Rq^n$ has cardinality $q^{Nn}$.  The cyclotomic
leftover-hash lemmas of Boudgoust and Lapiha~\cite{EPRINT:BouLap25} therefore imply the claimed
$2^{-\lambda}$ statistical distance once the source entropy exceeds the output entropy by $2\lambda$ bits.
\end{proof}

\begin{theorem}[LHL-based target hiding of $h_{\mathrm{tran}}$]\label{thm:htran-target-hiding}
Assume the hypotheses of Lemma~\ref{lem:htran-lhl}, so the public parameters are honestly generated and in particular $B_2$ is sampled uniformly at setup.  Let $r_1$ be any fixed admissible inverse-side
randomizer and define, for a bounded message $\mu\in\mathcal M$,
\[
  T_\mu := B_0+B_1\mu+B_2r_0+(B_3-r_1)^{-1}.
\]
Then $T_\mu$ is statistically $2^{-\lambda}$-close to uniform over $\Rq^n$.  Consequently, for any two bounded
messages $\mu_0,\mu_1\in\mathcal M$,
\[
  \Delta(T_{\mu_0},T_{\mu_1})\le 2^{1-\lambda},
\]
so the public BB-tran target statistically hides the message.
\end{theorem}

\begin{proof}
By Lemma~\ref{lem:htran-lhl}, the distribution of $B_2r_0$ is statistically $2^{-\lambda}$-close to a uniform
$U\getsunif\Rq^n$.  For fixed admissible $r_1$ and fixed message $\mu$, the remaining term
\[
  C_{\mu,r_1}:=B_0+B_1\mu+(B_3-r_1)^{-1}
\]
is a deterministic offset, hence
\[
  T_\mu=B_2r_0+C_{\mu,r_1}
\]
is also statistically $2^{-\lambda}$-close to uniform.  Applying the same argument to $\mu_0$ and $\mu_1$ and
using the triangle inequality yields the displayed bound.  No coefficient-norm bound on $(B_3-r_1)^{-1}$ is
used in this hiding argument: the inverse-side term enters only as an additive offset.
\end{proof}
...
protocol distribution of $r_0$ satisfies Definition~\ref{def:htran-long-r0}, and
Theorem~\ref{thm:htran-target-hiding} applies to the BB-tran target used during issuance.
\end{corollary}
...
instantiating the rational family with the BB-tran form of \eqref{eq:htran-target}.  Assume the admissibility,
predicate, denominator-bound, strong-linear-independence, and norm-compatibility side conditions required by
DKLW for BB-tran, and assume the corresponding strong hinted-vSIS hypothesis.  Then
...
\begin{corollary}[Imported route to sEUF-CMA]\label{cor:dkwl-seuf-cma}
Under the corresponding DKLW $\GenISIS_f/\IntGenISIS_f$ lifting for the BB-tran family, the same
non-blind signature layer $\Sigma_{\mathrm{tran}}$ inherits strong existential unforgeability under adaptive
chosen-message attack.
\end{corollary}

\begin{proof}
DKLW show that the BB-tran family enjoys the same interactive-GenISIS$_f$ lift as their general vSIS
signature template, which upgrades the selective-query guarantee to full strong unforgeability under
chosen-message attack.
\end{proof}
...
\]
is pseudorandom if oracle access to $\PRF(k,\cdot)$ for uniform $k\gets\mathcal K$ is computationally
indistinguishable from oracle access to a uniform random function from $\mathcal D$ to $\mathcal T$.
\end{definition}

We instantiate the PRF with a Poseidon2-style permutation.
...
For key $\veck\in\Fq^{\ell_{\mathsf{key}}}$ and public input $\vecn\in\Fq^{\ell_{\mathsf{nonce}}}$, define
\[
...
\begin{assumption}[Concrete Poseidon2 PRF assumption]\label{ass:poseidon-prf}
For the concrete parameter choices fixed in Appendix~\ref{app:prf}, the family
\[
  F(k,n)=\Tr_{\ell_{\mathsf{tag}}}\bigl(P(k\Vert n)+(k\Vert n)\bigr)
\]
of Definition~\ref{def:poseidon-prf} is a secure pseudorandom function.
\end{assumption}
