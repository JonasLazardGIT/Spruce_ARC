...
\]
In ARC-SPRUCE, the Ajtai commitment equation, centering, the inverse-witness equation, the BB-tran target
identity, the short-preimage signature equation, and the PRF checks are expressed as parallel constraints, while
range, carry, and packing-consistency gadgets may additionally use aggregated checks.
...
\begin{align}
  (B_3-r_1)\Had Z&=1, \label{eq:sw-show-inv}\\
...
Assume the public replay map $\mathcal F_{\mathrm{rep}}$ used by the showing implementation is additive,
multiplicative, and injective.
\end{lemma}

\begin{theorem}[Replay-space certification implies the coefficient-domain BB-tran identity]
\label{thm:replay-to-coeff}
Assume the compiled showing relation enforces the exact bridges above and the replay residual
\eqref{eq:sw-replay-residual}.  Then the extracted coefficient-domain witness satisfies
\[
  (B_3-r_1)\Had\bigl(\mA\vecu-B_0-B_1(\vecm\Vert\veck)-B_2r_0\bigr)=1
\]
in $\Rq$.
...
\begin{proof}
The inverse equation $(B_3-r_1)\Had Z=1$ is part of the compiled relation.  By
...
relation.  Selector constraints bind the PRF key lanes to the key rows $\veck$; checkpoint constraints certify the
nonlinear Poseidon2 S-box steps; linear layers and round constants are replayed from public parameters; and the
final feed-forward and truncation constraints bind the public pair $(\nonce,\prftag)$.
