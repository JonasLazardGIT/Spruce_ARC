...
The abstract quotient \(h_{m,(r_0,r_1)}(B)\) is evaluated only when \(B_3-r_1\in \Rq^\times\). In the issuance
model, this admissibility condition is enforced during honest target derivation by restart. The compiled pre-sign proof
of Section~\ref{subsec:presign-compiled} certifies only the cleared-denominator relation for the public target \(T\).
...
transform-domain cleared relation for the public target. The post-sign proof adds the signature shortness gadget, the
full replay-image showing relation, and the PRF checkpoint constraints.
...
assumption on a reduced replay map.
