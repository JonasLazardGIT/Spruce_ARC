...
\end{enumerate}
Combining existing post-quantum primitives generically, even with zero-knowledge friendly hash functions~\cite{EPRINT:PWFW23}, leads to non-competitive performance. Thus, existing works \cb{todo: cite} carefully choose commitment schemes, signatures and NIZKs such that the NIZK for the given relation has good proof size and prover/verifier time. However, the state-of-the-art is generally not considered efficient enough for practical deployments:
...
\lena{I'd add more motivation and explanation here- why vSIS and rational hashing, what rational hashing is etc. here}
...
\(\vecmu=\vecm\Vert \veck\), and additionally samples short \(\vecr_0^\Holder,\vecr_1^\Holder,\vecr\) to compute the
...
\[
\vect := h_{\vecmu,\left(\centerfunc{\vecr_0^\Holder+\vecr_0^\Issuer},\centerfunc{\vecr_1^\Holder+\vecr_1^\Issuer}\right)}(B),
\]
...
\lena{seems incorrect- to the holder?}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		% --- Notes and messages ---
		\node[note] at ($(H)+(0,-1.55)$) {Sample $\vecm,\veck$ and set $\vecmu=\vecm\Vert \veck$\\ (with $\veck$ as PRF key)\\ Sample $\vecr_0^\Holder,\vecr_1^\Holder,\bar \vecr$};
		...
		\node[note] at ($(H)+(0,-9.6)$) {Store credential\\ $(\vecm,\veck,\vecr_0,\vecr_1,\vecu)$};
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[
		...
		% --- Notes and messages ---
		\node[note] at ($(H)+(0,-2.5)$) {Choose unused $\nonce$\\ Compute $\prftag\leftarrow \PRF(\veck,\nonce)$ \\ Compute $\pi_{\mathsf{post}}$ proves:\\ $\exists(\vecu,\vecm,\veck,\vecr_0,\vecr_1)$ s.t.\\ $\vecmu=\vecm\Vert \veck$ and $\mA \vecu=h_{\vecmu,(\vecr_0,\vecr_1)}(B)$\\ and $\prftag=\PRF(\veck,\nonce)$
		};
...
\lena{I'm missing that the holder has some state in Figure 2}
