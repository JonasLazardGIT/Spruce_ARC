...
We use a Poseidon2-like permutation with feed-forward and truncation. Let \(k\in\Fq^{\ell_{\mathsf{key}}}\) be the secret
key (derived from the signed \(\veck\)) and \(n\in\Fq^{\ell_{\nonce}}\) encode the public nonce. Let
...
\end{itemize}
The truncation length is chosen to satisfy \(\ell_{\prftag}\ge \lceil \lambda/\log_2(q)\rceil\), ensuring that the
tag contains \(\lambda\) bits of entropy.
...
appendix is the concrete realization used for the present instantiation and measurements. Alternative checkpoint schedules
with the same interface could be substituted without changing the statement layer of
