...
\begin{enumerate}[leftmargin=1.25em]
  ...
  holder randomness distribution for the retained BB-tran target.
...
\]
Our issuance statement keeps \(T\) public but now also carries the two BB-tran
auxiliary product rows, so \(d_Q\) is driven by membership checks together with
those low-degree product constraints; the showing statement additionally
depends on the PRF effective degree (Appendix~\ref{app:prf}).
...
\]
with masking degree \(d_Q\) derived from the instantiated constraint degrees.
The pre-sign statement keeps \(T\) public and adds only the BB-tran auxiliary
product constraints, while the post-sign statement’s effective degree is still
dominated by the PRF S-box composition.
