...
$  (\TrapGen,\SampPre)
$
and admissible public parameters \((n,m,q,\BoundSig)\)  such that:
\begin{itemize}[leftmargin=1.25em]
...
\]
consisting of the lattice trapdoor suite above together with a setup and public evaluation algorithm. On input \(\SecParam\),
...
\(\Eval(B,m,\chi)\) takes a message \(m\in\mathcal{M}\) and witness randomness \(\chi\in\mathcal{X}\) and outputs a
...
\[
  ...
  \Pr_{\chi\gets \mathcal{X}}\big[\Eval(B,m,\chi)=\bot\big]\le \delta\right\}\text{.}
\]
Correctness requires that for
every \(m\in\mathcal{M}_B\) and every \(\chi\in\mathcal{X}\) with \(\Eval(B,m,\chi)=\vect\neq\bot\), the output
...
\cb{NEED: does $\vect$ hide anything about $m$?}

In this work, we use the retained BB-tran concrete relation from
\cite[Table~1]{EPRINT:DKLW25}, specialized to the T-preserving proof
architecture used by ARC-SPRUCE.

\begin{definition}[Rational hash, admissibility, and cleared form]\label{def:rational-hash}
Our concrete instantiation uses a public key \(B=(B_0,B_1,B_2,B_3)\in\Rq^4\),
message \(m\in\Rq\), and randomness \((r_0,r_1)\in\Rq^2\) with coefficient
bounds
\(\|m\|_\infty,\|r_0\|_\infty,\|r_1\|_\infty\le \BoundMsg\). Write
\[
\operatorname{den}(B,r_1):=B_3-r_1.
\]
In the retained 4-slot encoding we require \(B_0=0\).
...
When this admissibility condition holds, the concrete BB-tran target is the
partial function
\[
h^{\mathrm{bb\mbox{-}tran}}_{m,(r_0,r_1)}(B):=B_1m+B_2r_0+\operatorname{den}(B,r_1)^{-1}.
\]

For \(T\in\Rq\), we also define the cleared-denominator relation
\begin{equation}\label{eq:hash-cleared}
  B_3T-Tr_1-(B_3B_1)m-(B_3B_2)r_0+B_1(mr_1)+B_2(r_0r_1)-1=0.
\end{equation}
This is the expanded cleared form obtained by multiplying the BB-tran target
equation by \(B_3-r_1\). The implementation certifies this expanded form using
auxiliary product rows for \(mr_1\) and \(r_0r_1\).
\end{definition}
...
\[
\operatorname{den}(B,r_1)=(d_1,\ldots,d_s),\,
m=(m_1,\ldots,m_s),\,
r_0=(x_1,\ldots,x_s),\,
r_1=(y_1,\ldots,y_s),\,
B_j=(b_{j,1},\ldots,b_{j,s})\text{ for }j\in\{1,2,3\},\,
T=(t_1,\ldots,t_s).
\]
If \((B,m,r_0,r_1)\) is admissible, that is, \(d_i\neq 0\) for every \(i\), then
\[
T=h^{\mathrm{bb\mbox{-}tran}}_{m,(r_0,r_1)}(B)
\iff
B_3T-Tr_1-(B_3B_1)m-(B_3B_2)r_0+B_1(mr_1)+B_2(r_0r_1)-1=0.
\]
\end{lemma}
...
\begin{proof}
Under the CRT isomorphism, the BB-tran target relation is the coordinate-wise
statement
\[
t_i=b_{1,i}m_i+b_{2,i}x_i+d_i^{-1}
...
t_i=b_{1,i}m_i+b_{2,i}x_i+d_i^{-1}
\iff
d_it_i-d_i(b_{1,i}m_i+b_{2,i}x_i)-1=0.
\]
Substituting \(d_i=b_{3,i}-y_i\) and expanding gives
\[
b_{3,i}t_i-t_iy_i-b_{3,i}b_{1,i}m_i-b_{3,i}b_{2,i}x_i+b_{1,i}m_iy_i+b_{2,i}x_iy_i-1=0.
\]
Collecting the coordinates yields the equivalence. If admissibility fails, then
\(d_i=0\) in at least one slot. In that slot the expanded cleared equation no
longer determines \(t_i\) uniquely, so the abstract quotient remains undefined
there.
\end{proof}
%\cb{is this true? what if all terms are simply zero in one of the subfields?}

All later uses of the rational hash state explicitly whether they refer to the
abstract admissible BB-tran quotient relation or to the weaker expanded
cleared relation.
...
Note that the undefined case \(h_{m,(r_0,r_1)}(B)=\bot\) is exactly the inadmissibility event that \(B_3-r_1\) is not invertible
in $R_q$, and is modeled by the admissibility parameter \(\delta\). We will later choose  \((r_0,r_1)\) such that they are uniformly random within the bounded randomness space. 
...
\((B,\mathcal{M},\mathcal{X},\delta,\allowbreak\BoundMsg,\allowbreak\BoundSig)\). The associated signature scheme
...
Correctness requires that all admissible parameters, correctly generated $(\vk,\sk)$ and \(m\in\mathcal{M}_B\), the signatures generated using $\Sign$ are accepted by $\SigVerify$ except with
negligible probability.
...
We now define security as strong
existential unforgeability under random-message attacks.
\begin{definition}[Unforgeability for vSIS-style signatures]\label{def:vsis-games}
Let \(\TDRH\) be a trapdoor rational hash scheme  with parameters
\((B,\mathcal{M},\mathcal{X},\delta,\allowbreak\BoundMsg,\allowbreak\BoundSig)\) and
\(\vsisscheme=(\KeyGen,\Sign,\SigVerify)\) be the associated signature scheme.
Fix \(Q\in\poly(\lambda)\). In the experiment
\(\mathsf{Exp}_{\mathrm{sEUF\mbox{-}RMA}}^{\Sigma_{\mathrm{vSIS}},\adversary,Q}(\lambda)\), the challenger runs $(\vk,\sk)\gets \KeyGen$, then uniformly samples \(m_1,\ldots,m_Q \gets \mathcal{M}_B\) and computes \(\sigma_i\leftarrow \Sign(sk,m_i)\) for each $i\in [Q]$. Finally
...
\end{definition}
We can equivalently define \(\mathrm{sEUF\mbox{-}CMA}\), \(\mathrm{sEUF\mbox{-}SMA}\), and \(\mathrm{sSUF\mbox{-}SMA}\) security where $\mathrm{CMA}$-security allows the adversary to choose all $m_i$ itself after seeing $\vk$, while $\mathrm{SMA}$-security requires the adversary to pick all $m_i$ as well as $m^*$ \emph{before} seeing $\vk$.

\cb{CONTINUE FROM HERE}

In \cite{EPRINT:DKLW25}, the authors showed the following
\begin{lemma}\label{lem:vsis-unforgeability}
Let \(\ell_{\mathrm{hash}}=4\) and $
...
\begin{enumerate}[leftmargin=1.25em]
    ...
    present verifier also satisfies \(\|\vecu\|\le \beta_{\mathrm{DKLW}}\) in the norm used in \cite{EPRINT:DKLW25} \cb{Is this just $\BoundSig$? What norm is this?};
    \item the predicate condition of \cite[Definition~7]{EPRINT:DKLW25} holds for the instantiated rational-hash
    family \cb{what does this mean?}; and 
    \item the denominator-bound and strong-linear-independence side conditions required by
    \cite[Theorems~4--8 and Corollary~1]{EPRINT:DKLW25} hold for the instantiated family. \cb{what does this mean?}
\end{enumerate}
Then 
\begin{enumerate}[leftmargin=1.25em]
    \item The construction in Definition~\ref{def:vsis-template} is correct.
    \item Under the strong random-hinted vSIS assumption for the induced parameter tuple, the signature scheme is \(\mathrm{sEUF\mbox{-}RMA}\)-secure.
    \item For \(f(B,m,\chi)=h_{m,(r_0,r_1)}(B)\), the experiment \(\mathsf{GenISIS}_f\) is equivalent to the
    ...
    \item Under the strong-hinted vSIS assumption\cb{is it the strong-hinted or random strong-hinted? we might skip one of the two in the Appendix.}, the construction also enjoys
    \(\mathrm{sEUF\mbox{-}SMA}\) security.% by \cite[Theorem~5]{EPRINT:DKLW25}. 
    
\end{enumerate}
\end{lemma}

\begin{proof}
The claims follow from \cite[Theorems~4, 5, 7, 8, and Corollary~1]{EPRINT:DKLW25}.
\end{proof}
...
\begin{definition}[Blindness]\label{def:bs-blindness}
  Let $\mathsf{BSig}$ be a blind signatures scheme and $\mu_0 \neq \mu_1$ be any two messages. 
 %  any two messages
 %  \(\mu_0,\mu_1\) running the following experiment 
  $\mathsf{Exp}^{\mathsf{sig\mbox{-}blind}}_{\mathsf{BSig},\Adv}(\lambda)$ between a challenger $\Challenger$ and a three-stage stateful adversary $\Adv=(\Adv_1,\Adv_2,\Adv_3)$:
 \begin{enumerate}[leftmargin=1.25em]
   \item $\Challenger$ runs $\PublicParamsBS\gets \Setup(\SecParam)$.
   \item Run
		$\vk\gets \Adv_1(\PublicParamsBS)$
   \item Run
		$\Adv_2^{\Issue:\langle \cdot: \Holder(\mu_b,\vk)\rangle ,~ \Issue:\langle
    \cdot: \Holder(\mu_{1-b},\vk)\rangle }; $ at the end of which 
     $\Challenger$ obtains $\sig_0,\sig_1$.
   \item If $\bot\in\{\sig_0,\sig_1\}\text{ then
     }(\sig_0,\sig_1)\gets(\bot,\bot)$\lena{or output a random element from the
     signature space?}  \cb{maybe random bit, like in presentation unlinkability?}
   \item $\Challenger$ samples $b\getsunif\{0,1\}$ \cb{shouldn't this be sampled above? Also in the game, $\Adv$ gets both signatures} and outputs
   \begin{align*}
     \begin{cases}
     \sig_0 \leftarrow & \text{ if } b=0\\
     \sig_1 \leftarrow & \text{ if } b=1
     \end{cases}
   \end{align*}
   \item $\Adv_3(\sig_0,\sig_1)$ outputs $b'$.
 \end{enumerate}
 Define $\Adv^{\mathsf{sig\mbox{-}blind}}_{\mathsf{BSig}}(\Adv) := \big|
 \Pr[b'=b]-\tfrac12 \big|$.
...
\item $\Issue$ is an interactive protocol between $\Issuer$ and $\Holder$, where $\Issuer$ has input $\sk$ and $\Holder$ has input a message $\mu$ and verification key $\vk$. At the end of the protocol, $\Holder$ obtains the credential $\credential$.
  ...
  \item $\Verify(\vk,\presentation,\mu,\algstate)$ is a deterministic polynomial-time algorithm that on input a verification key $\vk$, a presentation $\presentation$, a message $\mu$ and a verifier state $\algstate$ outputs a validity bit together with an updated state $\algstate'$.
	 \end{itemize}
\end{definition}

Adjusting the blind-signature correctness notion of Definition~\ref{def:bs-correctness} and the blindness notion from Definition~\ref{def:bs-blindness} to ARC is immediate. 
...
outputs a sequence \((\presentation_i,\mu_i)_{i\in [K]}\). For \(i=1,\ldots,K\), the challenger computes
\[
(b_i,\algstate_i)\gets \Verify(\vk,\presentation_i,\mu_i,\algstate_{i-1}).
\]
...
This definition is the ARC analogue of one-more unforgeability. Since each honestly issued credential with some fixed $\mu$ can be shown for
all \(|\nonceSpace|\) different nonces, \(Q\) issuance interactions may legitimately yield \(Q\cdot |\nonceSpace|\)
accepting presentations. Producing strictly more than that forces a policy violation for at least one nonce or one $\mu$ by the
pigeonhole principle.
%\lena{this is already nonce-defined, maybe we want to keep the definition more general. This is basically one-more unforgeability with the additional constraint that we can only spend a credential once- should we make this two different definitions?} \cb{No I think it should be one only, as the nonce space naturally changes the unforgeability notion.}
