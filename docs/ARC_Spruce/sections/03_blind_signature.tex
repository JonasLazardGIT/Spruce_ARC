...
\]
is a trapdoor rational hash according to Definition~\ref{def:rational-hash}. In
the concrete instantiation used below, the public target \(\vect\) is the
retained admissible BB-tran value
\(h^{\mathrm{bb\mbox{-}tran}}_{\vecmu,(\vecr_0,\vecr_1)}(B)\), while the
compiled proof layer certifies the corresponding expanded cleared relation of
Definition~\ref{def:rational-hash}. The public setup will output the hash key
