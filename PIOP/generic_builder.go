		...
		if !skipConstraintRebuild {
			...
			if opts.Credential && len(constraintRows) > 0 {
				...
				if pub.IntGenISIS && len(pub.Com) > 0 && len(pub.CM) > 0 && len(pub.AS) > 0 && len(pub.A) == 0 {
					...
					postRows, cerr := buildIntGenISISShowingConstraintSetFromRows(ringQ, pub, rowLayout, constraintRows, omegaWitness, set.PRFCompanionLayout, opts.PhaseRecorder, opts)
					if cerr != nil {
						return nil, cerr
					}
					if rebuiltEmpty {
