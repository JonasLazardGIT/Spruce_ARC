...
func NewLinfChainSpec(q uint64, W, L, ell int, beta uint64) LinfSpec {
	...
	return LinfSpec{
		Q: q, R: R, W: W, L: L, Ell: ell, UsesAbsRow: true,
...
// NewLinfChainSpecCapped instantiates the membership-chain parameters for the
// ℓ∞ gadget while allowing tighter per-digit caps (β-aware gadgets).
//
// Caps apply to digits i>=1 (unsigned digits). If caps[i] is provided (>0),
// digit i is constrained to the range [0..caps[i]] instead of [0..R-1]. Digit 0
// remains balanced in [-2^{W-1}, 2^{W-1}-1].
//
// This is useful when the natural representable range of (W,L) is much larger
// than the intended bound β, and you want the gadget to enforce β (or a small
// slack) rather than merely be able to represent it.
func NewLinfChainSpecCapped(q uint64, W, L, ell int, beta uint64, caps []int) LinfSpec {
	if (q & 1) == 0 {
		panic("q must be odd")
	}
	if ell < 1 {
	...
	// Higher digits: unsigned range [0, cap] where cap defaults to R-1.
	for i := 1; i < L; i++ {
		cap := int(R) - 1
		...
		if cap < 0 || cap >= int(R) {
			panic(fmt.Sprintf("linf chain: invalid cap for digit %d: %d (R=%d)", i, cap, R))
		}
		dMax[i] = cap
		digitLo[i] = 0
		digitHi[i] = cap
		pols[i] = buildMembershipPolyRange(q, 0, cap)
	}
...
// NewSignedLinfChainSpec builds a signed-digit decomposition gadget with no
// auxiliary |P| row: P = Σ_i R^i·D_i and D_i constrained by per-digit ranges.
//
// Default digit ranges are balanced:
//   - digit 0: [-R/2, R/2-1]
//   - digit i>0: [-R/2, R/2-1]
//
// Optional caps tighten |digit_i|: if caps[i] > 0 then digit i is constrained
// to [-caps[i], caps[i]].
func NewSignedLinfChainSpec(q uint64, W, L, ell int, beta uint64, caps []int) LinfSpec {
	if W <= 0 {
		panic("W must be ≥ 1")
	}
	return NewSignedLinfChainSpecRadix(q, uint64(1)<<uint(W), L, ell, beta, caps)
}
